<article class="overflow-hidden rounded-[8px] border border-[#eadfe0] bg-white shadow-[0_14px_35px_rgba(28,25,23,0.05)]">
    ...
    <div class="grid gap-5 p-5 md:grid-cols-2">
        <x-material.floating-input name="summary" label="Resumo" :value="old('summary', $attendanceData['summary'])" placeholder="Ex.: Orientações sobre amamentação e pega correta" required-indicator wrapper-class="md:col-span-2" />
        <x-material.floating-textarea name="objective" label="Objetivo" :value="old('objective', $attendanceData['objective'])" required-indicator wrapper-class="md:col-span-2" />
        <x-material.floating-textarea name="complaint" label="Queixa principal" :value="old('complaint', $attendanceData['complaint'])" required-indicator />
        <x-material.floating-textarea name="evaluation" label="Avaliação" :value="old('evaluation', $attendanceData['evaluation'])" required-indicator />
        <x-material.floating-textarea name="conduct" label="Conduta" :value="old('conduct', $attendanceData['conduct'])" required-indicator wrapper-class="md:col-span-2" />
    </div>
</article>
<article class="overflow-hidden rounded-[8px] border border-[#eadfe0] bg-white shadow-[0_14px_35px_rgba(28,25,23,0.05)]">
